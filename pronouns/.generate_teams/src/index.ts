...
[...data.teams.values()].forEach((team) => {
  ...
  data.pronouns.single.forEach((single) => {
    setupTeams.push(
      `team join ${team.name}_${single} @s[scores={mpp_pronouns_select=${count}},tag=!mpp_afk,tag=exi_${team.name}]`
    );
    count++;
  });
  data.pronouns.pairs.forEach((x) => {
    data.pronouns.pairs.forEach((y) => {
      if (x != y) {
        setupTeams.push(
          `team join ${team.name}_${x}_${y} @s[scores={mpp_pronouns_select=${count}},tag=!mpp_afk,tag=exi_${team.name}]`
        );
        count++;
      }
    });
  ...
  data.pronouns.triples.forEach((x) => {
    data.pronouns.triples.forEach((y) => {
      data.pronouns.triples.forEach((z) => {
        if (x != y && y != z && x != z) {
          setupTeams.push(
            `team join ${team.name}_${x}_${y}_${z} @s[scores={mpp_pronouns_select=${count}},tag=!mpp_afk,tag=exi_${team.name}]`
          );
          count++;
        }
      });
  ...
  setupTeams.push(
    `team join ${team.name} @s[scores={mpp_pronouns_select=${count}},tag=!mpp_afk]`
  );
});
setupTeams.push(
