#Sleeping players
execute store result score value_sleepers mcp_sleep_count if entity @a[nbt={Dimension:"minecraft:overworld"},tag=mcp_in_bed]

#Total player count & Required player calculation
execute store result score value_players mcp_sleep_count if entity @a[gamemode=!spectator,nbt={Dimension:"minecraft:overworld"},tag=!mcp_afk]
