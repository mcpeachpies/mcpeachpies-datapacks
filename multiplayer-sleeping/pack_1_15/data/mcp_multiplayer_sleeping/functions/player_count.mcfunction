#Sleeping players
execute store result score value_sleepers mcp_sleep_count if entity @a[tag=mcp_in_bed,nbt={Dimension:0}]

#Total player count & Required player calculation
execute store result score value_players mcp_sleep_count if entity @a[gamemode=!spectator,tag=!mcp_afk,nbt={Dimension:0}]
